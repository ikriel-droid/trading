...
The operator checklist now also shows whether the latest release pack artifacts are missing, partial, invalid, or ready, and points you to `release-pack` / `release-verify` when distribution output needs attention.
...
`Release Center` also exposes a `Run Recommended` button that follows the current artifact state, checksum issues, and manifest validity, then triggers the suggested release workflow stage directly.
